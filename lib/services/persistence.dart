...
class DatabaseHelper {
  ...
  _initDb() async {
    return openDatabase(
      join(await getDatabasesPath(), 'mangas.db'),
      onCreate: (db, version) async {
        await db.execute(
          '''CREATE TABLE $_mgTable(
            ...
            $_mgScraperID TEXT DEFAULT "manganato" NOT NULL, 
  ...
  updateManga(Manga manga) async {
    ...
    await db!.transaction((txn) async {
      await txn.update(
          _mgTable,
          manga.toMap(),
        where: '$_mgID = ?', whereArgs: [manga.id],
      );
      for (var ch in manga.chapters) {
        if (ch.isNew()) {
          ch.mangaID = manga.id;
          ...
          await txn.update(
            _chTable,
            ch.toMap(),
            where: '$_chMangaID = ? AND $_chID = ?', whereArgs: [ch.mangaID, ch.id],
          );
        }
      }
    });
  ...
  Future<void> deleteManga(int mangaID) async {
    ...
    await db!.transaction((txn) async {
      txn.delete(
          _chTable,
        where: '$_chMangaID = ?', whereArgs: [mangaID],
      );
      txn.delete(
        _mgTable,
        where: '$_mgID = ?', whereArgs: [mangaID],
      );
    });
  }
  ...
  Future<Manga?> getManga(int id) async {
    final db = await database;
    var mangas = await db!.query(_mgTable, where: '$_mgID = ?', whereArgs: [id]);
    if (mangas.isEmpty) {
      return null;
    }
